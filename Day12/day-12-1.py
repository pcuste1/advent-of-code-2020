...
class Ship():
    ...
    def __init__(self):
        ...
        self.current_direction = 1 # represents initial heading east
    ...
    def preform_action(self, action, steps):
        ...
        if action == "W":
            self.ew -= steps
        if action == "F":
            self.preform_action(self.actions[self.current_direction], steps)
        if action == "R":
            self.current_direction = ( self.current_direction + ( steps // 90 ) ) % 4
        if action == "L":
            self.current_direction = ( self.current_direction - ( steps // 90 ) ) % 4
